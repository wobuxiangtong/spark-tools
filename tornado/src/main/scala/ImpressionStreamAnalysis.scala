...
object ImpressionStreamAnalysis extends AbstractAnalysis{
...
//    .defaultsTo(24)
  def main(args: Array[String]): Unit = {
    ...
    val checkPointDir = PropertyBag.getProperty("impression.checkpoint.dir", "")
  ...
  def functionToCreateContext()(implicit options:OptionSet): StreamingContext = {
    val sparkConf = new SparkConf().setAppName("Tornado Impression Stream")//.setMaster("local[2]")
    ...
    val kafkaParams = Map[String, Object](
      ...
      "group.id" -> "tornado-impression",
      "auto.offset.reset" -> "latest",
      "enable.auto.commit" -> (false: java.lang.Boolean)
    )
    val topics = Array("dot-impression")
...
//      .checkpoint(Seconds(options.valueOf(checkpointDuration).intValue()))
      ...
      rdd.foreachPartition(partition => {
        if(!partition.isEmpty){
          ProduceRowMsg.produceClick(producer,"analysis-result","impress",partition)
        }
      })
    }
    })
